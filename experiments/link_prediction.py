...
def load_link_prediction(model, dataset, batch_size):
    """
    ...
    with torch.no_grad():
        ...
        mrr, hits, ranks = eval(
            model=model, valset=testsub[:3], truedicts=truedict, n=len(i2n), r=len(i2r),
            batch_size=batch_size, verbose=True)

    print(f'MRR {mrr:.4}\t hits@1 {hits[0]:.4}\t  hits@3 {hits[1]:.4}\t  hits@10 {hits[2]:.4}')

    lp_results = {}
    lp_results[model_path] = []
    lp_results[model_path].append({'mrr': mrr,
                                'h@1': hits[0],
                                'h@3': hits[1],
                                'h@10': hits[2]})

    outfile_path = 'data/'+dataset+'/lp_results_{}.txt'.format(date.today().strftime("%Y%m%d"))
